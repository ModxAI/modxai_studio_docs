...
- **聊天模型**：文本对话模型（含 GGUF 和 HF Transformers 两种推理后端）
- **向量模型**：用于 RAG 检索或生成 RAG 数据集需要的嵌入模型
- **重排模型**：用于检索结果重新排序
- **多模态模型**：支持图文理解
- **SD 模型**：Stable Diffusion 图像生成模型
- **TTS 模型**：文本转语音模型
- **STT 模型**：语音转文本模型（基于 faster-whisper）
...
**推理后端**：
- **llama.cpp**（GGUF 格式）：无需安装环境，开箱即用，推荐大多数用户使用
- **HF Transformers**（HuggingFace 格式）：需安装 CPU 或 GPU 环境，支持更灵活的精度控制和设备选择

**格式支持**：
- `.gguf`：llama.cpp 量化格式（推荐）
- `.bin`：GGML 格式
- `.safetensors`：HF Transformers 格式

**环境要求**：llama.cpp 后端无需安装环境；HF 后端需在**设置**中安装环境
...
导入 HF Transformers 格式的聊天模型后，可在模型操作列进行以下配置：

| 参数 | 说明 | 默认值 | 选项 |
|------|------|--------|------|
| 推理设备 | 选择推理使用的设备 | auto | auto / cpu / cuda |
| 权重精度 | 模型加载时的数值精度 | 空（自动） | float16 / float32 / bfloat16 |

> HF 模型当前仅支持纯文本推理，不支持多模态（图像输入）。
...
⚠️ **默认Vulkan加速** 如果设置为**无限制上下文**选项时,可能会导致当前版本的llamacpp加载时报错. CUDA加速环境下不会出现此问题. 建议NVIDIA显卡用户下载CUDA加速库以获得最佳体验。

⚠️ **多模态模型不支持知识库问答（RAG）**。多模态对话模式下，RAG 检索功能不可用。如需使用知识库问答，请切换到纯文本聊天模型。
...
**格式支持**：
- `.pth`：Kokoro 系列模型权重文件
- 需配合 `config.json` 配置文件和 `.pt` 音色文件使用

**环境要求**：需要安装 CPU 或 GPU 环境

**支持的语言**：
- **美式英语（a）**：American English
- **英式英语（b）**：British English  
- **普通话（z）**：Mandarin Chinese

**特性**：
- 支持自动语言检测
- 支持多种预设音色（af_heart、am_adam 等）
- 支持语速调节（0.5x - 2.0x）
- 支持 CUDA 加速和 CPU 推理
...
**格式支持**：
- 基于 faster-whisper 引擎，模型文件通过下载器获取

**环境要求**：需要安装 CPU 或 GPU 环境（推荐 GPU）

**特性**：
- 支持多种计算精度（float32 / float16 / int8）
- 支持自动语言检测和指定语言
- 支持 VAD（语音活动检测）过滤静音
- 支持 CUDA 加速和 CPU 推理
...
2. **选择文件**：
   - **聊天/多模态**：选择 `.gguf` 或 `.bin` 文件；HF 格式选择包含 `.safetensors` 的目录
   - **向量/重排**：选择 `.safetensors` 文件或包含该文件的目录
   - **SD 模型**：需单独使用 SD 导入对话框
   - **TTS 模型**：选择 `.pth` 权重文件
   - **STT 模型**：通过下载器获取，模型文件自动管理
...
加载模型后，ModxAI Studio 会自动暴露兼容 OpenAI 协议的 HTTP API，供局域网内的其他应用或设备调用。

### 接口列表
| 端点 | 方法 | 说明 |
|------|------|------|
| `/v1/chat/completions` | POST | OpenAI 兼容的聊天推理接口 |
| `/v1/models` | GET | 列出当前可用模型 |

### 使用场景
- **局域网协作**：同一网络下的其他设备可直接调用本机的模型推理服务
- **第三方应用集成**：支持任何兼容 OpenAI API 的客户端（如其他智能体应用、自动化脚本等）直接接入
- **开发调试**：使用 curl、Postman 等工具直接测试推理接口

> 无需 API Key 认证。服务端口和地址可在应用内查看。
...
- **自动卸载**：加载新模型时会自动卸载旧模型
- **并发限制**：智能体不支持聊天和多模态同时加载。聊天、向量、重排、SD、TTS、STT 可同时加载
...
- **并发限制**：不支持聊天和多模态同时加载。聊天、向量、重排、SD、TTS、STT 可同时加载
