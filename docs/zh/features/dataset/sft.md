...
| 参数 | 说明 | 默认值 | 建议 |
|------|------|--------|------|
| **最小段落长度** | 低于此字符数的段落将被过滤 | 3 | 技术文档可设置为 5-10 |
| **样本作用范围** | 清洗规则的应用范围 | 结构 | `结构` - 仅清理结构区域<br>`全文` - 对全文应用 |
| **清洗样本** | 自定义需要清除的内容 | 空 | 用 `\|` 分隔多个样本 |
...
这是 SFT 数据处理的核心步骤，通过 AI 能力自动将原始文本/代码转换为结构化的数据。
...
**作用**：解析步骤 4 的 AI 输出，提取结构化数据。

此步骤会在步骤 4 完成后自动执行，将 AI 的自由文本输出解析为标准化格式。
...
> ⚠️ **重要警告**：
> 
> AI 过滤需要使用**分析能力较强的模型**（如 14B 以上参数量的模型）。如果使用能力不足的模型，可能会：
> - 错误地将高质量数据标记为低质量
> - 保留实际有问题的数据
> - 导致最终数据集质量下降
>
...
1. **合成步骤**：建议使用 7B 以上参数的模型
2. **过滤步骤**：建议使用 14B 以上参数的模型
3. **本地模型**：确保 llama.cpp 服务已启动
