...
在文件类型下拉框中选择「JSON」即可切换到结构化数据输入模式。适合已有固定字段的 JSON/JSONL 数据（如新闻、业务记录），可直接将字段映射到标准格式，无需走完整的非结构化清洗链路。

选择结构化数据后转换步骤节点会显示以下专属参数：

| 参数 | 说明 | 默认值 |
|------|------|--------|
| 限制数量 |仅处理前 N 个文件或记录，0 表示不限制| 0 |
| 记录 ID 字段 | 每条记录的唯一标识字段名 | id |
| 标题字段 | 映射到文档标题的源字段 | title |
| 正文字段 | 映射到文档正文的源字段（必填） | content |
| 摘要字段 | 映射到文档摘要的源字段 | description |
| 关键词字段 | 映射到关键词的源字段，支持逗号分隔多字段 | keywords,tag_key |
| 分类字段 | 映射到分类的源字段，支持逗号分隔多字段 | catname,areaname |
| 作者字段 | 映射到作者的源字段 | author_key |
| 来源字段 | 映射到来源的源字段 | copyfrom |
| 发布时间字段 | 映射到发布时间的源字段 | inputtime |
...
> 预训练不再有独立的步骤。在AI合成步骤节点的**导出格式**设为 `pretrain`，后续步骤会按预训练格式输出纯文本语料。
...
| **噪声样本** | 自定义需要清除的内容 | 空 | 用 `\|` 分隔多个样本，如：`联系我们`<br>建议每个样本不超过 20 个字符 |
...
| 参数 | 说明 | 默认值 | 建议 |
|------|------|--------|------|
| **开启质量过滤** | 是否使用嵌入模型进行质量过滤功能 | 开启 | 建议开启，提高数据质量. 如果出现误判，可以关闭此功能 |

#### 嵌入模型依赖

此步骤的近重复检测功能支持使用嵌入模型来提高检测准确度。系统默认尝试使用 `all-MiniLM-L6-v2` 模型,如果无法安装,则采用替代算法进行近重复检测。
...
此步骤的近重复检测功能支持使用嵌入模型来提高检测准确度。系统默认采用算法进行近重复检测。
...
预训练不再作为独立步骤在步骤（AI 合成）中设置**导出格式**参数即可控制最终输出：

| 导出格式 | 输出内容 | 适用场景 |
|----------|----------|----------|
| sft_chatml | ChatML 对话格式（system/user/assistant） | SFT 微调训练 |
| pretrain | 纯文本语料（单 assistant 消息） | 预训练 / 继续预训练 |
| dpo | 偏好对格式（prompt/chosen/rejected） | DPO 偏好对齐训练 |
...
| **导出格式** | 最终训练数据的输出格式 | sft_chatml |
| **思维链模式** | 注入 CoT 推理提示，引导模型输出思考过程 | 关闭 |

**导出格式选项**：`sft_chatml`（SFT ChatML 对话格式）、`pretrain`（预训练纯文本）、`dpo`（DPO 偏好对格式）。

**思维链模式**：开启后自动注入内容推理型 CoT 提示，并关闭模型原生的 thinking 功能以避免双重思考标签污染。适合需要模型展示推理步骤的分析任务。
...
| **启用思考** | 使用模型原生 thinking | 否 | - |
| **服务器地址** | 远端推理服务地址，格式 `http://ip:port/v1/chat/completions` | 空 | - |
