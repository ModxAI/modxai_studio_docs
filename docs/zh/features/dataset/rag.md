...
**结构化数据输入**：在文件类型下拉框中选择「JSON」，可直接导入已有固定字段的 JSON/JSONL 数据。转换步骤会显示字段映射参数（标题字段、正文字段、关键词字段等），将源字段映射到标准格式，跳过非结构化解析链路。详见 [SFT 数据处理 - 结构化数据](./sft.md#文本数据-text)。
...
- **重新导入**：支持重新导入已删除的集合，前提是向量数据文件仍然存在.需选择相应的导入文件(rag_dataset_***_meta.json)进行导入.

> 程序目录中包含根据官方手册生成的示例知识库, 您可以使用**重新导入**该知识库. 体验 RAG 功能. 
> 导入文件路径: <安装盘>/ModxAI Studio/ModxAI-Backend/_internal/server/resources/example/rag_manual/rag_dataset_reimport_meta.json
