...
**结构化数据集**（文件类型选择「JSON」）：
- 支持格式：JSON、JSONL
- 适用场景：新闻、表格、业务记录等已有结构化字段的数据，可直接映射字段避免走非结构化清洗链路
- 输出格式：标准化的 JSONL 训练数据
...
- **AI 合成**：包含 AI 合成步骤，适合需要扩充数据的场景
- **AI 质量**：包含 AI 合成和 AI 过滤步骤，适合追求高质量数据
- **自定义**：手动选择需要执行的步骤

> 预训练不再有独立的步骤。在步骤 4（AI 合成）中将**导出格式**设为 `pretrain`，会自动按预训练格式输出。
...
SFT 或 RAG 数据处理中的 AI 节点支持复用已加载的聊天模型：

- 如果聊天界面已加载模型，且配置（基础模型 + LoRA）与 AI 节点需要的一致
- 系统会自动检测并复用该模型，请求进入队列排队处理
- 多个请求通过队列机制串行处理，互不干扰, 支持边聊边处理
- **任务完成后不会卸载模型**，您的聊天会话不受影响
