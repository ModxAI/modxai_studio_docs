...
在环境类型（CPU/GPU）之外，还需选择**安装能力**来决定安装哪些依赖组件：

| 安装能力 | 包含内容 | 适用场景 |
|----------|----------|----------|
| **全部** | 训练 + 智能体 + TTS 全部依赖 | 需要使用所有功能的用户 |
| **智能体** | 核心库 + Playwright 浏览器环境 + Bun 运行时 | 仅需使用智能体的网页浏览和 Web 工具 |
| **语音合成** | 基础库 + misaki G2P + spaCy 模型 | 仅需 TTS 语音合成功能 |

> 安装能力可在安装确认对话框中切换。不同能力安装的依赖包范围不同，选择合适的可节省磁盘空间和安装时间。
...
选择「全部」或「智能体」安装能力时，会额外安装以下智能体专用依赖：

| 组件 | 用途 | 说明 |
|------|------|------|
| Playwright | 浏览器自动化 | 智能体网页浏览工具的运行环境 |
| Bun | JS/TypeScript 运行时 | 智能体 Web 工具的执行引擎 |

> ⚠️ **智能体使用浏览器和 Web 工具的前置条件**：必须安装「全部」或「智能体」安装能力。如果仅安装「语音合成」能力，智能体的浏览器相关工具将无法使用。
...
TTS（语音合成）功能需要以下依赖。根据安装能力不同，安装方式有所差异：
...
**全部模式（异步安装）**：
- misaki 和 spaCy 模型在主环境完成后**后台异步下载**
- 安装完成后需**等待一段时间**，刷新环境状态后确认 TTS 能力是否可用
- 异步下载失败不影响主环境安装成功的判定

**语音合成模式（同步安装）**：
- misaki 和 spaCy 模型在主流程中**同步安装**，直至完成或失败
- 适合异步下载失败后，利用此模式独立修复 TTS 依赖
- 安装完成后立即刷新即可确认状态

> ⚠️ **TTS 依赖安装说明**：
> - misaki 库强制使用 **PyPI 官方源**安装，因国内镜像源对 extras 依赖解析不完整
> - 中国内地用户下载可能较慢或失败，请保持网络稳定
> - 异步安装失败不影响主环境，可通过切换到「语音合成」模式独立重装修复
...
> 💡 **时间说明**：实际安装时间取决于网络速度和选择的镜像源。中国内地用户建议选择（阿里云/腾讯云）镜像源通常更快。
