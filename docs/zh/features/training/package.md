...
量化模块负责将训练完成的模型（包括基础模型和 LoRA 权重）转换为适合推理部署的 GGUF 格式。由于参数较多且存在互斥和依赖关系，**强烈建议使用预设模板**进行操作。
...
> 💡 **建议**：由于量化选项之间存在复杂的依赖关系，强烈建议使用预设模板。

> ⚠️ **注意**：Lora权重和基础模型的路径不能包含中文等特殊字符，以免导致量化失败。

### 模板一览

| 模板 | 说明 | 适用场景 |
|------|------|----------|
| **合并并量化（默认）** | 合并 LoRA + 导出 GGUF + 量化 Q4_K_M | SFT 训练后的标准量化流程 |
